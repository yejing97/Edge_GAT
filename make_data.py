...
def make_npz(tgt_path, inkml, lg, stroke_emb_nb, rel_emb_nb, speed, norm, max_size):
...
def make_batch_npz(tag, root_path, tgt_path, stroke_nb, rel_nb, speed, norm, max_size):
    for root, _, files in os.walk(os.path.join(root_path, tag)):
        for file in files:
            if file.endswith('.inkml'):
                ...
                try:
                    make_npz(os.path.join(tgt_path, tag), inkml, lg, stroke_nb, rel_nb, speed, norm, max_size)
                except:
...
def make_data(inkml_path, npz_path, stroke_emb_nb, rel_emb_nb, speed, norm, max_size):
    make_batch_npz('train', inkml_path, npz_path, stroke_emb_nb, rel_emb_nb, speed, norm, max_size)
    make_batch_npz('val', inkml_path, npz_path, stroke_emb_nb, rel_emb_nb, speed, norm, max_size)
    make_batch_npz('test', inkml_path, npz_path, stroke_emb_nb, rel_emb_nb, speed, norm, max_size)
    ...
    for s in S:
        for r in R:
            ...
            if not os.path.exists(npz_path):
                make_data(inkml_path, npz_path, s, r, False, 'stroke', 200)